(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))n(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function o(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(r){if(r.ep)return;r.ep=!0;const s=o(r);fetch(r.href,s)}})();async function p(e){try{const t=new URL("http://localhost:8080/get-questions/");t.searchParams.append("name",e);const o=await fetch(t,{method:"GET",headers:{Accept:"*/*"}});if(!o.ok)throw o.status===0?new Error("Network error: Please check if the server is running and CORS is enabled"):new Error(`Server responded with status: ${o.status}`);const n=await o.json();return Array.isArray(n)?n:[]}catch(t){throw t instanceof TypeError&&t.message==="Failed to fetch"?new Error("Unable to connect to the server. Please check if it's running and CORS is enabled."):(console.error("Error fetching questions:",t),t)}}function m(e,t){if(!t.length){f(e);return}const o=document.createElement("ul");o.className="question-list",t.forEach((n,r)=>{const s=h(n,r);o.appendChild(s)}),e.innerHTML="",e.appendChild(o)}function h(e,t){const o=document.createElement("li");o.className="question-item",o.style.animationDelay=`${t*.05}s`,o.innerHTML=`
    ...
    `;const n=o.querySelector(".copy-button");return n.addEventListener("click",r=>{r.stopPropagation();const s=n.dataset.question;s&&(y(s),g(n))}),o}function f(e){e.innerHTML=`
    ...
    `}function y(e){navigator.clipboard.writeText(e).catch(t=>{console.error("Failed to copy text:",t)})}function g(e){const t=e.innerHTML;e.innerHTML=`
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M5 12L10 17L20 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
      Copied!
    `,e.style.color="var(--color-success-500)",setTimeout(()=>{e.innerHTML=t,e.style.color=""},2e3)}const a="code_explainer_user_name";function v(e){localStorage.setItem(a,e)}function w(){return localStorage.getItem(a)}function C(){const e=document.getElementById("app");if(!e)return;e.innerHTML=`
    <header>
  ...
  `;const t=e.querySelector("#logout");t?t.addEventListener("click",()=>{console.log("Logging out..."),localStorage.removeItem("user"),localStorage.removeItem("token"),localStorage.clear(),window.location.reload()}):console.error("Logout button not found!"),q();const o=w();o&&(c(),l(o),u(o)),E()}function q(){const e=document.getElementById("user-input-form"),t=document.getElementById("user-input");!e||!t||e.addEventListener("submit",o=>{o.preventDefault();const n=t.value.trim();n&&(v(n),c(),l(n),u(n))})}function c(){const e=document.getElementById("user-input-container");e&&(e.style.display="none")}function l(e){const t=document.getElementById("questions-container"),o=document.getElementById("questions-content");!t||!o||(t.style.display="block",o.innerHTML='<div class="loading"><div class="loading-spinner"></div></div>',p(e).then(n=>{m(o,n),d(n.length)}).catch(n=>{o.innerHTML=`
        <div class="error">
          <p>Sorry, we couldn't load your questions. Please try again later.</p>
          <p>${n.message}</p>
        </div>
      `}))}function u(e){const t=document.getElementById("user-name-display");t&&(t.textContent=e)}function d(e){const t=document.getElementById("questions-count");t&&(t.textContent=`${e} question${e!==1?"s":""}`)}function E(){const e=document.getElementById("search-input");e&&e.addEventListener("input",t=>{const o=t.target.value.toLowerCase(),n=document.querySelectorAll(".question-item");n.forEach(s=>{(s.querySelector(".question-text")?.textContent?.toLowerCase()||"").includes(o)?s.style.display="block":s.style.display="none"});const r=Array.from(n).filter(s=>s.style.display!=="none").length;d(r)})}document.addEventListener("DOMContentLoaded",()=>{C()});
//# sourceMappingURL=index-D6iWSBRq.js.map
